...
for index in df2.index:
    ...
    if index in df1.index:
        ...
        if df2.loc[index, col_name] == df1.loc[index, col_name]:
            print(f'Key {index}: Yes')
            # result_df = result_df.append({'Key': index, 'Value': 'Yes'}, ignore_index=True)
            result_df.loc[len(result_df)] = [index, 'Yes',df2.loc[index, 'MATNR'],df2.loc[index, 'BWKEY'], df1.loc[index, col_name], df1.loc[index, col_name]]
        else:
            print(f'Key {index}: No')
            # result_df = result_df.append({'Key': index, 'Value': 'No'}, ignore_index=True)
        ...
        # result_df = result_df.append({'Key': index, 'Value': 'Key Not Found'}, ignore_index=True)
